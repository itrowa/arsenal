...
static StateMachine stateMachine = CREATE_STATE_MACHINE(onCommand, offCommand, stateCheckCommand, commandError, operandError);
...
static void stateCheckCommand(BYTE channel) {
  ...
  putUSBUSART(buf, sizeof(buf));
}

static void commandError(void) {
  putrsUSBUSART("COMMAND ERROR\r\n");
}

static void operandError(void) {
  putrsUSBUSART("OPERAND ERROR\r\n");
}
...
void ProcessIO(void) {
    ...
    if (USBUSARTIsTxTrfReady()) {
      ...
      onStringOnState(&stateMachine, USB_Out_Buffer, numBytesRead);
    }
